...
use stable_mir::ty::{FnDef, GenericArgKind, GenericArgs, MirConst, RigidTy, Ty, TyConst};
...
use uninit_visitor::{mk_const_operand, CheckUninitVisitor, InitRelevantInstruction, MemoryInitOp};
...
impl TransformPass for UninitPass {
    ...
    fn transform(&mut self, tcx: TyCtxt, body: Body, instance: Instance) -> (bool, Body) {
        ...
        let first_source = if !new_body.blocks()[0].statements.is_empty() {
            SourceInstruction::Statement { idx: 0, bb: 0 }
        } else {
            SourceInstruction::Terminator { bb: 0 }
        };
        let bless_args_instruction = InitRelevantInstruction {
            source: first_source,
            before_instruction: (1..=new_body.arg_count())
                .map(|arg_local| MemoryInitOp::SetRef {
                    operand: Operand::Copy(Place::from(arg_local)),
                    count: mk_const_operand(1, first_source.span(new_body.blocks())),
                    value: true,
...
pub fn resolve_mem_init_fn(fn_def: FnDef, layout_size: usize) -> Instance {
    Instance::resolve(
        fn_def,
        &GenericArgs(vec![GenericArgKind::Const(
            TyConst::try_from_target_usize(layout_size as u64).unwrap(),
        )]),
    )
    .unwrap()
}
